...
<meta name="theme-color" content="#712cf9">
  ...
  <%
		...
		/* if(!userID.equlas(animal.getUserID())){
			PrintWriter script = response.getWriter();
			script.println("<script>");
...
<main class="form-signin w-25" style="margin:150px auto!important;">
  ...
  <form method="post" action="updateAction.jsp?animalID=<%=animalID%>">
    <h1 class="h3 mb-3 fw-normal text-center">글 수정</h1>
	
    <div class="form-group">
     <label class="fw-bolder">* 글제목 </label>
      <input type="text" class="form-control"  name="animalTitle" value="<%=animal.getAnimalTitle()%>">
    </div>
    <div>
      <label class="fw-bolder">* 잃어버린 장소 </label>
     <select name="animalLost" class="form-control input-sm"  value="<%=animal.getAnimalLost()%>">                                            
    ...
    <div class="form-group">
     <label class="fw-bolder">* 연락처 </label>
      <input type="text" class="form-control"  name="userPhone"  value="<%=animal.getUserPhone()%>">
    </div>
    <div class="form-group">
     ...
     <label class="radio-inline">
         <input type="radio" name="animalGender" value="숫컷" />숫컷
     </label>
     <label class="radio-inline">
    ...
    <button onclick="return confirm('수정하시겠습니까?')" class="w-100 btn btn-lg btn-primary" style="margin-top:20px;" type="submit">수정하기</button>
